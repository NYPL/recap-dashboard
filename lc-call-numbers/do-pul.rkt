...
(define *ALL-PULS* (find-files (lambda (x) #t) (~> "../scsb-data/PUL/"
                                                   (string->path))))


(define *HOWMANY* (length *ALL-PULS*))
(define *COUNTER* 0)

(define PULOUT (open-output-file "../computed-data/lc-call-numbers/pul-lc-calls.txt" #:exists 'error))

(for/list ((afile (cdr *ALL-PULS*)))
  ...
  (for/list ((item ((sxpath "/bibRecords/bibRecord/bib") curr)))
    ...
    (fprintf PULOUT "~A\t~A\t~A~%" bibid oh50 oh90)
    (flush-output)))
